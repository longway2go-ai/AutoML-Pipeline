            ...
            with training_container:
                ...
                try:
                    ...
                    if problem_type == 'classification':
                        ...
                        cv_scores = cross_val_score(best_model, X_train_use, y_train, cv=cv_folds, scoring='accuracy')
                        results[name] = {
                            'model': best_model,
                        ...
                        cv_scores = cross_val_score(best_model, X_train_use, y_train, cv=cv_folds, scoring='r2')
                        results[name] = {
                            'model': best_model,
                            'best_params': random_search.best_params_,
                            'best_score': random_search.best_score_,
...
def step_final_results():
    """Step 6: Final Results and Model Insights"""
...
"""
    ...
    with col1:
        st.download_button(
            ...
            data=summary,
            file_name=f"automl_results_{best_model_name}.txt",
            mime="text/plain",
    ...
    with col2:
        ...
        st.download_button(
            ...
            data=results_csv,
            file_name=f"all_model_results.csv",
            mime="text/csv",
            use_container_width=True
        )
...
def main():
    # Header
    st.markdown('<div class="main-header">🤖 AutoML Pipeline</div>', unsafe_allow_html=True)
    st.markdown("### Complete Machine Learning Pipeline with Hyperparameter Optimization ✨")
    ...
    step_descriptions = {
        "📁 Data Upload": "Upload CSV/Excel files and basic data preview",
        "⚙️ Preprocessing": "Automated data cleaning and preparation", 
        "📊 EDA": "Comprehensive analysis with pie charts, box plots, and visualizations",
        "🚀 Model Training": "Train multiple ML models with hyperparameter tuning",
        "📈 Evaluation": "Compare model performance and detailed analysis",
        "🏆 Results": "Final insights, facts, and downloadable results"
    }
    
    cols = st.columns(3)
    for i, (step, description) in enumerate(step_descriptions.items()):
        with cols[i % 3]:
            st.write(f"**{step}**")
